...
- **Account system** — Optional sign-in (email + password or Google). After results, “Save your progress” opens the auth modal: create account (email, Create password, Confirm password) or sign in. Anonymous game data can be merged into the account. Account page: score history (rank only), profile records (e.g. extreme consensus). See `docs/account-system-design.md`, `docs/account-testing-and-debugging.md`, and `docs/account-system-test-checklist.md`.
...
1. **Environment:** Create `.env.local` in the project root with at least `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY`. For the account merge flow, add `SUPABASE_SERVICE_ROLE_KEY`. For AI axis suggestions, add `ANTHROPIC_API_KEY` (see `docs/ai-setup.md`). For email confirmation, configure custom SMTP in Supabase or see `docs/supabase-email-setup.md` and `docs/free-smtp-setup-guide.md`.
